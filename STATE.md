...
**Last updated:** 2026-07-17 (**M2 started: WP-21 metadata schema landed.** `tools/meta.schema.json` (JSON Schema draft 2020-12) is the meta contract, and the gate's new check J validates every meta against it, ten checks now. Implements the accepted RFC-0001; recorded as [ADR 0016](docs/internal/decisions/0016-adopt-machine-checkable-metadata-schema.md) and [ADR 0017](docs/internal/decisions/0017-gate-may-use-jsonschema-for-meta-validation.md). Earlier the same day: M1 complete, v0.1.0 tagged with a dogfooded release note that produced DF-1; TX-1, TX-2, D2, D3 closed)
...
| **Governance gate** | `tools/check-bundles.py`, **ten checks** (files, dashes, nesting incl. heading level, clean example, citations **in both directions**, meta contract + placeholder scan, frontmatter YAML validity, history-documents-version, pairs_with/related_templates resolution, and **meta-schema validation**). Eight are pure stdlib; G uses PyYAML ([ADR 0014](docs/internal/decisions/0014-gate-may-use-pyyaml-for-frontmatter-validity.md)) and J uses PyYAML plus jsonschema ([ADR 0017](docs/internal/decisions/0017-gate-may-use-jsonschema-for-meta-validation.md)), each SKIPping locally if its dependency is absent. **Runs in CI** on every push and PR; branch protection on `main` requires it to pass before merge. Passing on all six bundles. |
| **Decision records** | `docs/internal/decisions/`, seventeen ADRs in [MADR v4](https://github.com/adr/madr) format, plus a README documenting the convention. All accepted. Matches the org standard used by `agent-config-toolkit` and scaffolded by `jp-init-project`. |
...
**M2, the machine layer, is under way.** Its first item landed 2026-07-17:

| WP | What | State |
|---|---|---|
| **WP-21** | Metadata schema (`tools/meta.schema.json`; gate check J validates every meta against it) | **Done 2026-07-17 (PR #20).** The schema encodes `phase` XOR `classification` (ADR 0015) and codifies the 18 fields the six metas carry; check J enforces it in CI, adversarially tested against a 37-case battery hardened by an independent red-team. Recorded as [ADR 0016](docs/internal/decisions/0016-adopt-machine-checkable-metadata-schema.md) (the schema) and [ADR 0017](docs/internal/decisions/0017-gate-may-use-jsonschema-for-meta-validation.md) (the jsonschema dependency). The worked RFC example [RFC-0001](templates/rfc/rfc_example.md) *was* this proposal and is now accepted, written from into ADR 0016. |
| **WP-20** | HY-2 graduation ADR | **Next.** Scaffold already flat (ADR 0009); mostly a formalization. |
| **WP-22..28** | Machine catalog (incl. the generated `index.json` RFC-0001 also proposed), selection metadata, family contract, fill tooling, docs tree, v0.2.0 | Sequenced after WP-20. |
