...
**Last updated:** 2026-07-13 (M0 credibility floor executed; HY-2 scaffold graduation closed; gate size-contract rework; decision records conformed to MADR v4)
...
| **Governance gate** | `tools/check-bundles.py`, six checks (files, dashes, nesting, clean example, citation resolution, meta size contract). **Runs in CI** on every push to `main` and every pull request. Passing. |
| **Decision records** | `docs/internal/decisions/`, eleven ADRs in [MADR v4](https://github.com/adr/madr) format, plus a README documenting the convention. All accepted. Matches the org standard used by `agent-config-toolkit` and scaffolded by `jp-init-project`. |
