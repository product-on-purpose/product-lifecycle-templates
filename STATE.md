...
| **Governance gate** | `tools/check-bundles.py`, **nine checks** (files, dashes, nesting incl. heading level, clean example, citations **in both directions**, meta contract + placeholder scan, frontmatter YAML validity, history-documents-version, pairs_with/related_templates resolution). Eight are pure stdlib; G uses PyYAML and SKIPs locally if absent ([ADR 0014](docs/internal/decisions/0014-gate-may-use-pyyaml-for-frontmatter-validity.md)). **Runs in CI** on every push and PR; branch protection on `main` requires it to pass before merge. Passing on all six bundles. |
| **Decision records** | `docs/internal/decisions/`, fifteen ADRs in [MADR v4](https://github.com/adr/madr) format, plus a README documenting the convention. All accepted. Matches the org standard used by `agent-config-toolkit` and scaffolded by `jp-init-project`. |
