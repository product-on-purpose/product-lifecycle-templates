...
| What | State |
|---|---|
| **Bundles** | **Nine bundles: 8 of the 27 Tier-1 catalog types, plus `rfc`** (catalog 48, Tier 2, `must_have: false`, built early as the third `decision-docs` member). **19 Tier-1 types remain**, per the floor build-out; see D-D in [`buildout-specs.md`](docs/internal/buildout-specs.md) for the reconciliation. Family `delivery-docs` (**complete**): `prd`, `user-stories`, `product-backlog`, `sprint-backlog`, `acceptance-criteria`, `release-notes`. Family `decision-docs` (**complete**): `adr`, `rfc`, `sdd`. Eight files each. Status `beta`, `template_version` 0.1.0. |
| **License** | Apache-2.0, granted at the repo root. Copyright Jonathan Prisant. |
| **Governance gate** | `tools/check-bundles.py`, **eleven checks** (files, dashes, nesting incl. heading level, clean example, citations **in both directions**, meta contract + placeholder scan, frontmatter YAML validity, history-documents-version, pairs_with/related_templates resolution, **meta-schema validation**, and **family-contract conformance on either taxonomy axis**). Check K gates whichever of `phase` or `classification` a family's contract names (ADR 0015), so a standing family is one registry entry, not new check code; because that branch has no live subject until governance-docs lands, it carries a fixture-based self-test ([`tools/test-check-k.py`](tools/test-check-k.py), 31 assertions, mutation-checked) run in CI. Nine are pure stdlib; G uses PyYAML ([ADR 0014](docs/internal/decisions/0014-gate-may-use-pyyaml-for-frontmatter-validity.md)) and J uses PyYAML plus jsonschema ([ADR 0017](docs/internal/decisions/0017-gate-may-use-jsonschema-for-meta-validation.md)), each SKIPping locally if its dependency is absent. **Runs in CI** on every push and PR; branch protection on `main` requires it to pass before merge. Passing on all nine bundles. |
...
| **Decision records** | `docs/internal/decisions/`, twenty-two ADRs (through 0022) in [MADR v4](https://github.com/adr/madr) format, plus a README documenting the convention. All accepted. Matches the org standard used by `agent-config-toolkit` and scaffolded by `jp-init-project`. |
...
That heading was true from 2026-07-16 until 2026-07-22. **One thing is broken as of 2026-07-22**, and this is the record of it:

- **The public atlas claims four bundles are built. Nine are.** [`atlas/atlas.html`](atlas/atlas.html) ships a "Built" filter driven by a `built` boolean per type, and only `prd`, `user-stories`, `acceptance-criteria` and `release-notes` carry it; `adr`, `rfc`, `sdd`, `product-backlog` and `sprint-backlog` are all missing. The cause is structural, not clerical: the type data is **duplicated** (inline in `atlas.html` and again in [`atlas/catalog-data.json`](atlas/catalog-data.json)), **generated by nothing**, and **checked by nothing**. `gen-manifest.py --check` keeps `manifest.json` and the README count marker honest; no equivalent covers the atlas, so it drifted silently on the one surface that is public. Same defect class as audit finding G-01. The fix is a generator plus a `--check`, not five hand-edited booleans in two files, so it is its own work item rather than a footnote to a correction PR.
...
- ~~No family contract adopted.~~ **Both families now have adopted contracts.** [delivery-docs](docs/internal/contracts/delivery-docs.md) (2026-07-20, #24, [ADR 0020](docs/internal/decisions/0020-adopt-delivery-docs-family-contract.md)) and [decision-docs](docs/internal/contracts/decision-docs.md) (2026-07-21, [ADR 0022](docs/internal/decisions/0022-adopt-decision-docs-family-contract.md)) are both enforced by gate check K (phase, status, size shape, and that the contract resolves); all nine bundles conform. The registry (`FAMILY_CONTRACTS`) generalized to a second family as one entry, no new check code. Methodology stays descriptive in both, not gated ([ADR 0020](docs/internal/decisions/0020-adopt-delivery-docs-family-contract.md) lesson).
