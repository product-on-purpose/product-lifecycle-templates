...
<!-- counts: bundles=35, tier1=25, tier1remaining=2, statebuilt=35, statecandidate=168, stateoutofscope=2, commoncandidates=53, adrs=60, cisteps=32, checkk=112, checkformats=98, checklogs=97, logsgated=29, sourcesgated=1247 -->

**Last updated:** 2026-09-25 (**`v0.14.0` ships `announcement-internal-comms` and `change-request`, the 34th
and 35th bundles**, as a minor release the maintainer directed the same evening they merged. It also carries
the finish of ADR 0055, the last "nobody but the author has filled a template" lines retired from live
surfaces, and the catalog corrections for both new types.
The previous entry read: **`announcement-internal-comms` and `change-request` become the 34th and 35th
bundles, both `delivery-docs` members.** The internal announcement goes to `delivery-docs`, not the
`communication-docs` family that forecast it, by [ADR 0059](docs/internal/decisions/0059-announcement-internal-comms-joins-delivery-docs.md);
the change request fitted no family contract as written, so [ADR 0060](docs/internal/decisions/0060-change-request-joins-delivery-docs.md)
widened `delivery-docs`' membership test with the verb "changes", the first admission here that widens a
...
base is the widest of any Tier-2 type**: PMI, APM, PRINCE2 and the European Commission's PM² guide define it
by name, and PM² publishes its field list under CC BY 4.0. The sources disagree on what an issue *is*, so the
template makes a team state its threshold. **Every quotation in its research log was checked against the
source's raw text**, and the check caught a research dimension that had misreported PM²'s own field list. The
review's chaining lens, fixed in `v0.12.0` to read sibling examples, found six places where the new example
disagreed with them on its first run. Not in any release yet.
The previous entry read: **`v0.12.0` ships `launch-coordination-checklist`, the 31st bundle.** A
...
which settles the family's membership question from the type's own origin. The appendix is licensed
CC BY-NC-ND, so the bundle teaches the practice around it and adapts none of its items. **Every research
quotation was checked against the source's raw text, and 19 of 193 were not there.** The build cost
**$14.22 at API list rates**, all 15 agents on Sonnet, and answered the open question about the measured
builds' Opus drafting: those builds never ran the committed workflow at all.
The previous entry read: **`v0.11.2`: the build-cost reports `v0.11.0` shipped counted every
...
transcripts they come from are machine-local and absent on the runner. Every workflow agent label now
carries its bundle type, so a subagent identifies itself instead of being inferred from its own
prompt. **The recurring shape held again**: four stale claims were found while verifying, including
this file reporting twenty-seven bundles under the heading "Built and true today" when its own gated
marker nine lines above said thirty, the third instance of that defect in that one table.
The previous entry read: **`v0.9.0` and `v0.10.0`, on one day.** `v0.9.0` gave the library a
...
| **Agent surface** | Two skills (`plt-fill-template`, `plt-grade-doc`), two generated artifacts ([`manifest.json`](manifest.json), [`sections.json`](sections.json)), and an **MCP server** at [`tools/mcp_server.py`](tools/mcp_server.py) declared through [`.mcp.json`](.mcp.json) ([ADR 0045](docs/internal/decisions/0045-the-mcp-server-is-python-and-lives-in-this-repository.md), 2026-09-06). Five tools; all 71 template variants addressable; `validate_fill` and `stamp_and_strip` **call** the existing Python tools rather than reimplementing them, so they cannot disagree with the CLI. Gated by 49 assertions as CI step 29. *(Corrected 2026-09-25: this cell read 58 variants and 44 assertions, both true when the server shipped and stale since; neither figure carries a marker.)* **Not validated: no agent outside this repository has used it, and no assertion scores its ranking.** Python, not the sketched TypeScript npm package, because the plugin install already clones this repository so there is nothing to embed; the cost, named in the ADR, is that `pm-skills-mcp` and this now differ. |
| **Decision records** | `docs/internal/decisions/`, sixty ADRs (through 0060) in [MADR v4](https://github.com/adr/madr) format, plus a README index (drift-checked by `tools/check-adr-index.py`). **All accepted.** *(Corrected 2026-09-15: this cell read "forty-five ADRs (through 0045)" while the gated marker at the top of this file said 50. DF-5 for the seventh time, and the exact shape this file's own line 324 records from the `adrs=34` to `adrs=35` bump: the marker is gated and the prose beside it is not, so `check-counts.py` went green over a six-record gap. The marker is the only thing a machine reads here.)* [0038](docs/internal/decisions/0038-what-the-circularity-signature-obliges.md) sat `proposed` from 2026-08-08 to 2026-08-14, the only record in this library ever to do so, and was accepted with option C (adopt nothing) consciously rejected rather than skipped. Two records were added 2026-08-14: [0039](docs/internal/decisions/0039-maintainer-discretion-replaces-the-pull-gate.md) **amends grow-by-pull so the maintainer may build any template at discretion**, and [0040](docs/internal/decisions/0040-free-and-open-source-no-paid-tier.md) closes VL-1 as **free and open source with no paid tier**. Added 2026-08-22: [0041](docs/internal/decisions/0041-maintainer-preference-sets-the-build-order.md) **makes the maintainer's own preference and need set the build order**, amending 0039's retention of the queue as a priority signal, on the ground that a ranking input which has never been non-zero cannot rank; it names its own cost, that it removes the last structural brake against building content nobody uses. Added 2026-09-03: [0043](docs/internal/decisions/0043-the-usage-gate-becomes-advisory.md) **makes the usage gate advisory**, downgrading 0041's forty-bundle falsifier to a soft reminder, rewording the roadmap's risk-table entry from an instruction to an observation, and ungating M5. **It moved no honesty mechanism**: `beta` until a real usage cycle, never calling a bundle proven on a green gate, and publishing zero real fills as zero all stood at the time. *(Two of those three were retired 2026-09-21 by [ADR 0055](docs/internal/decisions/0055-retire-the-zero-fills-disclosure.md): the fill-count disclosure and the usage-cycle graduation rule. **The claim ban survives and is now gate-enforced on the site**, so the sentence describes what 0043 decided, not what is in force.)* Its stated principle is that a library may grow without evidence but may not claim without evidence, and it names its own cost, that it removes the last structural pressure toward outreach. Added 2026-09-05: [0044](docs/internal/decisions/0044-the-section-schema-is-a-second-generated-artifact.md) **makes `sections.json` a second generated artifact beside the manifest**, following 0018, and makes its parser the first mechanical reader of the Approach A guidance grammar, so a malformed comment now fails a build that would previously have passed. It records three measured departures from the 22-line AG-1 sketch and corrects that sketch's extraction regex, which extracts zero of 353 WHAT fields. The last three family contracts ([ADR 0032](docs/internal/decisions/0032-adopt-standing-standards-family-contract.md), [0033](docs/internal/decisions/0033-adopt-process-docs-family-contract.md), [0034](docs/internal/decisions/0034-adopt-communication-docs-family-contract.md)) ratified 2026-08-05. Added 2026-09-22: [0056](docs/internal/decisions/0056-build-cost-is-measured-and-ingestion-is-separate-from-verification.md) **makes per-bundle build cost a measurement rather than an estimate**, and separates ingestion from verification because the harness transcripts it reads are machine-local and absent on the CI runner, so the gate checks an index it can re-derive and never the reports it cannot. Added 2026-09-23: [0057](docs/internal/decisions/0057-issue-log-joins-governance-docs-as-a-fourth-member.md) admits `issue-log` to `governance-docs` as a fourth member, settling the family question before its spec rather than inside its build, and [0058](docs/internal/decisions/0058-definition-of-ready-joins-standing-standards-as-a-foundation.md) admits `definition-of-ready` to `standing-standards` as a `foundation`. Added 2026-09-25: [0059](docs/internal/decisions/0059-announcement-internal-comms-joins-delivery-docs.md) sends `announcement-internal-comms` to `delivery-docs` rather than the `communication-docs` family that forecast it, because that family's only axis value describes a standing instrument, and [0060](docs/internal/decisions/0060-change-request-joins-delivery-docs.md) admits `change-request` by widening `delivery-docs`' membership test with the verb "changes", **the first admission that widens a test rather than applying one**: no contract admitted a per-occasion change request as written. Matches the org standard used by `agent-config-toolkit` and scaffolded by `jp-init-project`. |
...
| **Layout** | The library lives at `templates/` (flat, by document type), the gate at `tools/`, the atlas at `atlas/`, the measured per-bundle build-cost reports at `bundle-builds/`, and the planning, strategy, catalog, roadmap and decision records at `docs/internal/`. Decision HY-2 (scaffold graduation) closed 2026-07-12; the `_local/` split closed 2026-07-14 ([ADR 0013](docs/internal/decisions/0013-local-split-and-going-public.md)). |
| **Atlas** | 205-type interactive catalog map at `atlas/atlas.html`. |
| **Build harness** | The runbook at [`bundle-pipeline.md`](docs/internal/bundle-pipeline.md) is executable: [`.claude/commands/build-bundle.md`](.claude/commands/build-bundle.md) drives it and [`.claude/workflows/build-bundle.js`](.claude/workflows/build-bundle.js) runs its three stages: the research fan-out, the drafting fan-out that writes seven of the eight bundle files, and the review fan-out. *(Corrected 2026-09-22: this cell read "its research and review fan-outs", the same two-versus-three error corrected in `bundle-pipeline.md` the same day. Nothing gates a sentence against the script it describes.)* Review lenses read one brief, [`review-standards.md`](docs/internal/review-standards.md), which states what CI already proves so they do not re-prove it. Two report-only lints (`lint-number-provenance`, `lint-unsourced-confidence`) run at phase 3.5; a source cache (`tools/source-cache.py`, `_local/`) makes a page fetched once per bundle rather than once per agent. Every agent label is now prefixed with the bundle type (`${type}/draft:companion` rather than `draft:companion`), so each subagent is self-identifying in the run journal. [`tools/gen-bundle-build-report.py`](tools/gen-bundle-build-report.py) turns those journals into measured build-cost reports under `bundle-builds/`, replacing an estimate ("0.6-1M tokens" / "700K-1M tokens") that was wrong by more than an order of magnitude: the two whole-build measurements taken so far are `test-summary-report` at 24,650,553 and `spike-report` at 20,743,317 weighted token-equivalents. |
...
- **A green gate is a structural claim, not a quality one.** Every filled artifact in the repo is an authored example, and every bundle is `beta`. The gate proves structure and research integrity; it does not prove a document helped anyone.
...
- **The 2026-08-05 agentic-era research, parked with its errors named (2026-08-14).** A 58 KB strategy
  ...
  repository: it describes 15 bundles and 27 decision records against today's 30 and 54, it recommends
...
`<!-- counts: bundles=30, tier1=24, adrs=35 -->`. The check recomputes each from the tree and fails on
...
**No bundle is called "proven", "verified" or "validated" on the strength of a green gate**, and that restraint is gate-enforced on the site and mutation-tested. [ADR 0047](docs/internal/decisions/0047-the-usage-precondition-leaves-the-language-too.md) removed the scheduling gate and left the claim discipline standing; [ADR 0055](docs/internal/decisions/0055-retire-the-zero-fills-disclosure.md) retired the separate requirement to *publish a fill count*, which was a negative disclosure rather than a guard against a false claim. Nothing waits on a usage signal; nothing is *claimed* without one either. All 33 bundles are `beta`.
...
- **Still on credit:** "reference implementation", for a library that covers a minority of its own 205-type catalog. Efficacy is measured rather than argued as of 2026-08-08, and the first measurement returned **VOID** ([`evals/results/`](evals/results/)), which is a weaker position than "unmeasured" rather than a stronger one.
