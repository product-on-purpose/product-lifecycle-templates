...
| What | State |
|---|---|
| **Bundles** | **7 of 27 Tier-1 catalog types** (one down, 20 to go, per the floor build-out). Family `delivery-docs`: `prd`, `user-stories`, `acceptance-criteria`, `release-notes`. Family `decision-docs`: `adr`, `rfc`, `sdd`. Eight files each. Status `beta`, `template_version` 0.1.0. |
