...
| What | State |
|---|---|
| **Bundles** | **9 of 27 Tier-1 catalog types** (three down, 18 to go, per the floor build-out). Family `delivery-docs` (**complete**): `prd`, `user-stories`, `product-backlog`, `sprint-backlog`, `acceptance-criteria`, `release-notes`. Family `decision-docs` (**complete**): `adr`, `rfc`, `sdd`. Eight files each. Status `beta`, `template_version` 0.1.0. |
