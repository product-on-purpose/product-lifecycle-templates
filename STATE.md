...
| What | State |
|---|---|
| **Bundles** | **8 of 27 Tier-1 catalog types** (two down, 19 to go, per the floor build-out). Family `delivery-docs`: `prd`, `user-stories`, `product-backlog`, `acceptance-criteria`, `release-notes`. Family `decision-docs`: `adr`, `rfc`, `sdd`. Eight files each. Status `beta`, `template_version` 0.1.0. |
