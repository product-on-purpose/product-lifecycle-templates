...
**Last updated:** 2026-07-18 (**M2 machine layer: WP-21 (schema), WP-22 (manifest), WP-23 (selection metadata) all landed.** Every meta is schema-validated (gate check J, ten checks); `manifest.json`, generated by `tools/gen-manifest.py` and freshness-checked in CI, is the machine catalog an agent selects from, now carrying `default_size`, `sizing_guidance`, and a generated `approx_tokens` estimate per variant. Recorded as [ADR 0016](docs/internal/decisions/0016-adopt-machine-checkable-metadata-schema.md) through [ADR 0019](docs/internal/decisions/0019-selection-metadata-and-approx-tokens.md). WP-20 (graduation) confirmed already done via ADR 0009. M1 complete and v0.1.0 tagged 2026-07-17; TX-1, TX-2, D2, D3 closed)
...
| **Decision records** | `docs/internal/decisions/`, nineteen ADRs in [MADR v4](https://github.com/adr/madr) format, plus a README documenting the convention. All accepted. Matches the org standard used by `agent-config-toolkit` and scaffolded by `jp-init-project`. |
...
| **WP-24..28** | Family contract, fill tooling, freshness automation, docs tree, v0.2.0 | Next up: WP-24 (a `delivery-docs` family contract + gate check), then fill tooling toward v0.2.0. |
