...
**Last updated:** 2026-07-17 (**M2: WP-21 (metadata schema) and WP-22 (machine catalog) both landed.** `tools/meta.schema.json` + gate check J validate every meta (ten checks now); `manifest.json`, generated by `tools/gen-manifest.py` and freshness-checked in CI, is the machine catalog an agent selects from. Recorded as [ADR 0016](docs/internal/decisions/0016-adopt-machine-checkable-metadata-schema.md), [ADR 0017](docs/internal/decisions/0017-gate-may-use-jsonschema-for-meta-validation.md), [ADR 0018](docs/internal/decisions/0018-machine-catalog-generated-manifest.md). WP-20 (graduation) confirmed already done via ADR 0009. Earlier the same day: M1 complete, v0.1.0 tagged; TX-1, TX-2, D2, D3 closed)
...
| **Machine catalog** | [`manifest.json`](manifest.json) at the repo root: every bundle's selectable fields (`id`, `title`, `summary`, `doc_type`, `phase` or `classification`, `family`, `sizes_available`, `status`, `tags`, `aliases`) as one structured surface an agent selects from. **Generated** by [`tools/gen-manifest.py`](tools/gen-manifest.py) from the metas, committed, and kept fresh by CI (`gen-manifest.py --check` fails on drift or a stale README count marker). WP-22, [ADR 0018](docs/internal/decisions/0018-machine-catalog-generated-manifest.md). |
| **Decision records** | `docs/internal/decisions/`, eighteen ADRs in [MADR v4](https://github.com/adr/madr) format, plus a README documenting the convention. All accepted. Matches the org standard used by `agent-config-toolkit` and scaffolded by `jp-init-project`. |
...
| **WP-23..28** | Selection metadata, family contract, fill tooling, freshness automation, docs tree, v0.2.0 | Next up: WP-23 (selection metadata) extends the manifest with sizing guidance and token counts. |
