...
Five mechanisms in this repository generate a fact from the tree rather than accept it as typed prose, and
...
- **`sections.json`**, the AG-1 section schema, generated by [`tools/gen-sections.py`](../../tools/gen-sections.py)
  from the template variant files themselves: every section, its heading level, the sizes that carry it, the
  guidance fields its comment declares, whether the shipped body holds a table, and its placeholders. It
  ships beside `manifest.json` rather than inside it because embedding measured 4.6x on the artifact agents
  read to *select* a bundle. It is also the only thing in this repository that reads the Approach A guidance
  comments mechanically, so a comment it cannot parse is a malformed template no other check would report,
  and it fails rather than skipping the section: a section missing from the schema makes a downstream
  completeness check pass for the wrong reason.
...
None of the four generators claims to verify truth. `gen-research-log.py`'s docstring states it directly:
...
`tools/check-counts.py` takes a different approach from the four generators above: instead of generating
