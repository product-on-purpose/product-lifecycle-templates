...
<!-- counts: bundles=32 -->
All thirty-two bundles pass all eleven checks today. GitHub Actions runs the gate on every push and every
...
- **Two fixture-driven self-tests** for check K (family conformance) and the format axis. Most of their
  ...
  implementation.
  <!-- counts: checkk=106 -->
  `tools/test-check-k.py` runs 106 assertions;
  <!-- counts: checkformats=92 -->
  `tools/test-check-formats.py` runs 92.
...
- **The research-log contract** (`tools/check-research-logs.py`, plus its own self-test). Every source in
  ...
  `Supports:` clause.
  <!-- counts: logsgated=26, sourcesgated=1126 -->
  This runs across all 26 research logs the check gates, covering 1,126 individual sources.
...
- **Workflow prompt strings** (`tools/check-workflow-prompts.py`) and the **research-log generator
  ...
  <!-- counts: checklogs=94 -->
  The generator's own self-test runs 94 assertions.
