...
<!-- counts: cisteps=30 -->
Thirty CI steps run in total, and they are not all the same kind of thing. **Four** are checkout,
runtime setup and dependency installation, and prove nothing at all. **Twenty-five** prove the tree is
...
**The thirtieth is the only one that can surprise anybody**, because it is the only one whose rules
