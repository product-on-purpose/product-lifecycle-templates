...
<!-- counts: bundles=35 -->
All thirty-five bundles pass all eleven checks today. GitHub Actions runs the gate on every push and every
...
- **Two fixture-driven self-tests** for check K (family conformance) and the format axis. Most of their
  ...
  implementation.
  <!-- counts: checkk=112 -->
  `tools/test-check-k.py` runs 112 assertions;
  <!-- counts: checkformats=98 -->
  `tools/test-check-formats.py` runs 98.
...
- **The research-log contract** (`tools/check-research-logs.py`, plus its own self-test). Every source in
  ...
  `Supports:` clause.
  <!-- counts: logsgated=29, sourcesgated=1247 -->
  This runs across all 29 research logs the check gates, covering 1,247 individual sources.
...
- **Workflow prompt strings** (`tools/check-workflow-prompts.py`) and the **research-log generator
  ...
  <!-- counts: checklogs=97 -->
  The generator's own self-test runs 97 assertions.
