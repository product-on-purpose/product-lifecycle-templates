...
<!-- counts: bundles=33 -->
All thirty-three bundles pass all eleven checks today. GitHub Actions runs the gate on every push and every
...
- **Two fixture-driven self-tests** for check K (family conformance) and the format axis. Most of their
  ...
  implementation.
  <!-- counts: checkk=108 -->
  `tools/test-check-k.py` runs 108 assertions;
  <!-- counts: checkformats=94 -->
  `tools/test-check-formats.py` runs 94.
...
- **The research-log contract** (`tools/check-research-logs.py`, plus its own self-test). Every source in
  ...
  `Supports:` clause.
  <!-- counts: logsgated=27, sourcesgated=1159 -->
  This runs across all 27 research logs the check gates, covering 1,159 individual sources.
...
- **Workflow prompt strings** (`tools/check-workflow-prompts.py`) and the **research-log generator
  ...
  <!-- counts: checklogs=95 -->
  The generator's own self-test runs 95 assertions.
