...
|---|---|---|
| **Claude Code plugin** | both skills **and** all 32 bundles | you want the library to work. **This is the recommended route** |
...
Ask your agent to list the available document types. It should read `manifest.json` and name 32 bundles.
...
fetch any of the 67 template variants, fetch a grading pack, validate a filled document, and strip and
...
> Responses now also arrive as typed `structuredContent`, so a caller no longer has to re-parse JSON
> out of a text field. **This is a breaking change** for anything written against the older shapes.
...
**This installs the skills and not the library.** Eight files land, about 47 KB, and none of the 32
...
Everything is present: 32 bundles, the research logs behind every claim, the gate, the evals. Nothing
