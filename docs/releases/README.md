...
- [`v0.14.0.md`](v0.14.0.md) - two bundles in one release: an internal announcement, which tells the people who did not do the work what a launch means for them, and a change request, which asks for something already agreed to change and records the decision; the last fill-count lines are retired
- [`v0.13.0.md`](v0.13.0.md) - two bundles in one release: an issue log, the record of problems that have already happened on a project and who is fixing each, and a definition of ready, a team's agreement on when a backlog item can start, which carries its own sources' argument that a team may be better off without one. Every research quotation in both was checked against the source's raw text.
