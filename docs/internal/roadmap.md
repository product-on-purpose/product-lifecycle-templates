...
> | **WP-32 Demand capture** | **Half built.** `.github/ISSUE_TEMPLATE/` holds `new-type.md`, `usage-report.md` and `correction.md`, shipped 2026-08-07. **Zero issues have been filed**, the three labels those templates declare (`new-type`, `usage`, `correction`) do not exist in the repository, and `docs/reference/pull-queue.md` does not exist - all three re-verified 2026-09-11 against the remote and the tree. **The fourth item on this list was wrong and is struck:** it said `catalog-data.json` "still carries `built` with no `state`", and every one of the 205 catalog types has carried a generated `state` since 2026-08-22, as the Direction paragraph in `STATE.md` says in so many words. The row was refreshed 2026-08-14 and the field landed eight days later, so the claim aged into falsehood rather than being written wrong. The remainder is specified in [`pull-queue-spec.md`](pull-queue-spec.md) |
