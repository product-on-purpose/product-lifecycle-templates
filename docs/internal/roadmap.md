...
| WP-51 | AG-2 MCP server | `product-lifecycle-templates-mcp` per spec: list/select/fetch/grade/fill tools, bundle-summary resources, token-budgeted responses. **Spec refreshed 2026-09-05 at [`ag2-mcp-spec.md`](ag2-mcp-spec.md); the 2026-07-12 sketch had seven false claims and acceptance criteria no payload can meet.** Two of its five tools already exist as Python here. **Blocked on one maintainer decision: the stack** (a separate TypeScript npm package as the sketch says, or Python in this repository) | L+ | C-01 follow-on; `ag2-mcp-spec.md` |
