...
> | **WP-31 First real usage cycle** | **Not started.** Still the most valuable thing that could happen to this library, and **not a blocker for anything** ([ADR 0047](decisions/0047-the-usage-precondition-leaves-the-language-too.md)). It no longer holds bundles at `beta` either: since [ADR 0055](decisions/0055-retire-the-zero-fills-disclosure.md), `beta` to `stable` is the maintainer's judgement |
