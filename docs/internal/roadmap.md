...
>
> This file was written 2026-07-10 and last cites **ADR 0020**; there are now **58** decision records, and
> thirty-eight of them (0021 through 0058) postdate it and change the plan it describes.
>
...
>
> **Third time, 2026-09-20.** The marker went red on 52 versus 54 and was corrected. The sentence two
> lines above it, inside this very banner, still said **40** decision records and named the range
> `0021 through 0041`, both stale by fourteen records. **The paragraph documenting DF-5 had itself
> drifted**, and the marker did not catch it because a marker gates a marker. The gate's own advice,
> printed on every run, is the thing that found it: do not just edit the marker, re-read the prose
> around it.

<!-- counts: adrs=58, bundles=32 -->
...
| M5 Reach | Agents can discover, select, fetch, fill, validate | 4-6 weeks | MCP + fill flow live; distribution per D2/D3 outcomes | **Not started, and no longer gated.** [ADR 0043](decisions/0043-the-usage-gate-becomes-advisory.md) removed the M3-signal precondition; it may start whenever the maintainer wants it | C-01, E-02 remainder, AG-1, AG-2 |
...
| WP | Work package | Deliverables | Effort | Traces / spec |
|---|---|---|---|---|
| WP-30 | LP-2 grade-my-doc | `skills/plt-grade-doc/SKILL.md` (renamed from the spec's `grade-doc` by [ADR 0036](decisions/0036-library-prefix-and-skill-under-skills.md)) + rubric extraction per spec; report-card output format; works on all 26 bundles | L | D-05 path, E-04; `specs/spec_lp2-grade-my-doc.md` |
...
> | WP | State |
> |---|---|
> | **WP-30 LP-2 grade-my-doc** | **Built 2026-08-19, and not yet validated.** `skills/plt-grade-doc/` ships the skill, the report-card format, the grading procedure, the EV-3 form and one real worked example. **Two deviations from the spec are stated in the skill itself**: the `plt-` prefix ([ADR 0036](decisions/0036-library-prefix-and-skill-under-skills.md)), and a 0/1/2 scale the skill supplies for the 12 guides whose rubric is a checklist, applied under [decision procedure 5](decision-procedures.md) and routed to [`guide-rubric-spec.md`](guide-rubric-spec.md) section 4. **Its first run found a real defect** (ADR 0021 never pointed at the record amending it). **Most of its acceptance criteria are not run**, because they need documents this repository does not have. **This is the wedge**, and it is still the only item in this roadmap that delivers something to a person who has not adopted the library |
> | **WP-31 First real usage cycle** | **Not started. Zero fills by anyone but the author.** Still the most valuable thing that could happen to this library, and **not a blocker for anything** ([ADR 0047](decisions/0047-the-usage-precondition-leaves-the-language-too.md)). What its absence still does, through the honesty gate rather than through any schedule, is keep all 27 bundles at `status: beta` |
> | **WP-32 Demand capture** | **Built 2026-09-11**, as five of the six deliverables in [`pull-queue-spec.md`](pull-queue-spec.md). D1: the three labels (`new-type`, `usage`, `correction`) exist on the remote, where before there were only GitHub's nine defaults. D2: the three `.md` intake templates are now **issue forms**, every word of their prose preserved in `markdown` blocks, with the five fields the demand rule consumes as discrete inputs. D3: [`docs/reference/pull-queue.md`](../reference/pull-queue.md) states the rule and the four states and links three live filtered views, reachable from `README.md`, `CONTRIBUTING.md` and `choosing-a-template.md`. D4 (the `state` field) and D5 (the demand-rule ADR, which is [0041](decisions/0041-maintainer-preference-sets-the-build-order.md)) were **already done** and the spec had not recorded it. **D6 could not be built as written and that is a finding, not a gap:** it asked for `adr` to be seeded as `state: queued`, and by the time D4 shipped, `state` is derived from disk and an override contradicting a built bundle is a hard error - so the self-pull example lives on the queue page instead of as a false value in a generated file. **Still zero issues filed**, which is published on the page rather than omitted |
...
| WP | Work package | Deliverables | Effort | Traces / spec |
|---|---|---|---|---|
| WP-50 | LP-1 use-template flow | Interview-driven fill (skill first, CLI later) reusing ASK lines; stamps provenance; strips guidance; runs validation. **Mechanical half done 2026-09-05 (#125):** `tools/strip-template.py` and `tools/validate-fill.py`, wired into `plt-fill-template` as step 6. The interview half already shipped as guidance in that skill. **The spec's own comment-grammar regex is wrong** and extracts zero of 353 `WHAT` fields; the corrected rule is in `tools/gen-sections.py` | L | C-01; `specs/spec_lp1-use-template-flow.md` |
| WP-51 | AG-2 MCP server | **Built 2026-09-06.** [`tools/mcp_server.py`](../../tools/mcp_server.py), five tools, declared through a plugin-root [`.mcp.json`](../../.mcp.json), gated by a CI self-test - **which did not gate anything until 2026-09-11, and this row said it did.** The workflow installed `mcp` unpinned; the SDK's v2 renamed `FastMCP` to `MCPServer` on 2026-07-28, six weeks before this shipped; so CI got a 2.x the server cannot import, `test-mcp-server.py` SKIPPED its one SDK-dependent assertion, printed `OK 43 assertion(s) passed` and exited 0. **No CI run has ever verified that this server starts.** Fixed by pinning `mcp<2` in the workflow and by `--require-sdk`, which makes that skip exit non-zero. The stack decision is [ADR 0045](decisions/0045-the-mcp-server-is-python-and-lives-in-this-repository.md): **Python here, not the sketch's TypeScript npm package**, because the recommended install already clones this repository so there is nothing to embed. `validate_fill` and `stamp_and_strip` **call** the existing tools rather than reimplementing them. Building it falsified three further spec claims, including one of ours: the taxonomy axis is `phase` XOR `classification`, so the specified phase-only filter left 10 of 27 bundles unreachable. **Not validated:** no agent outside this repository has used it, and no assertion scores its ranking | L+ | C-01 follow-on; `ag2-mcp-spec.md` |
...
| WP-53 | AG-1 section schema | Generated per-bundle section schema (required/optional, tables, columns) consumed by LP-1/LP-2 validation. **Done 2026-09-05 (#123):** `sections.json`, 241 sections and 181 frontmatter fill sites, generated by `tools/gen-sections.py` with a `--check` mode in CI. Its parser is also the guidance grammar's first mechanical reader, so a malformed comment now fails a build. [ADR 0044](decisions/0044-the-section-schema-is-a-second-generated-artifact.md) records three measured departures from the AG-1 sketch | M | AG-1; machine-metadata spec section 6 |
...
| WP-55 | Per-bundle build-cost measurement | **Done 2026-09-22 (#172).** [`tools/gen-bundle-build-report.py`](../../tools/gen-bundle-build-report.py) and [`bundle-builds/`](../../bundle-builds/): one report per bundle per `template_version`, broken down by stage, model, requested tier and deliverable, generated from the harness transcripts and gated by a `--check` mode in CI beside the manifest and section schema. **22 reports backfilled.** Retrofitted here after the fact, like WP-51 and WP-53, because the work arose from a question rather than from this plan. **It replaced two estimates that disagreed with each other and were both low by roughly 10 to 20 times** ("0.6-1M tokens" and "700K-1M" against a measured 10M to 12M weighted token-equivalents, or $33 to $41 at API list rates), and corrected the runbook's claim about where the cost goes: drafting is the largest stage, not the research fan-out and review. *(Corrected 2026-09-22, in `v0.11.2`: this row read "20 to 30 times" and "21M to 25M", from a generator that counted each API response two to three times. All 22 reports re-ingested; see the Correction in ADR 0056.)* [ADR 0056](decisions/0056-build-cost-is-measured-and-ingestion-is-separate-from-verification.md) records the one deliberate weakness: **ingestion is separate from verification and CI cannot check the reports it gates**, because the transcripts are machine-local. **Not measured:** reasoning effort, which the harness records nowhere | M | DF-5; ADR 0041's cost input |
...
| Wedge outreach stalls (no real users found) | WP-33 log shows under 5 attempts by end of week 4 | **Observation, not an instruction, since [ADR 0043](decisions/0043-the-usage-gate-becomes-advisory.md).** Outreach is cheap per contact because of the EV-3 form, and building content is not a violation of anything. Earlier wording here read as a prohibition and was quoted as one |
