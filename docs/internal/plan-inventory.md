...
<!-- counts: bundles=32, adrs=57, cisteps=32 -->
...
<!-- counts: bundles=32 -->
Thirty-two researched bundles, a machine-readable `manifest.json`, four user-facing how-to documents, and
...
instructions and none of the 32 bundles, and each skill fetches what it needs at run time from the release
...
| `spec_ag2-mcp-server.md` (untracked) | MCP server: five tools, three resources, token budgets | **Superseded and built 2026-09-06.** Refreshed against the tree as [`ag2-mcp-spec.md`](ag2-mcp-spec.md), which found seven of the sketch's claims false; built as `tools/mcp_server.py` under [ADR 0045](decisions/0045-the-mcp-server-is-python-and-lives-in-this-repository.md), which found three more. Do not build from the sketch | M5. **No longer gated on M3**: [ADR 0043](decisions/0043-the-usage-gate-becomes-advisory.md) made the usage gate advisory |
...
| [`site-s0-spec.md`](site-s0-spec.md) | S0 of the site: the toolchain, the generator, the guards, and a deployed page | **Written 2026-09-19, ready to execute**, and unusual here in that it was **written after building a throwaway scaffold rather than before**. Twenty numbered acceptance criteria. Section 4 carries four findings the tracked plan does not contain, each observed in a real Astro build: the reference implementation does not guard the artifact it deploys, the family Node floor is below what the dependency tree demands, Starlight emits `rel="shortcut icon"` rather than `rel="icon"`, and `.gitattributes` covered none of the five file shapes the site introduces | **No decision.** Every gating decision is taken: [ADR 0046](decisions/0046-the-site-is-astro-starlight-under-pattern-s.md), [0047](decisions/0047-the-usage-precondition-leaves-the-language-too.md), [0051](decisions/0051-site-dependency-updates-follow-the-family-dependabot-pattern.md), plus the URL and branding calls recorded in the spec's own header. Pages is enabled |
