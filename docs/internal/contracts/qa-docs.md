...
**2026-08-07:** relabelled an unsourced CLAIM in section 2's axis rationale (the regression suite / ADR longevity comparison) as a POSITION the library states as its own reasoning, per decision-procedures.md procedure 11. No source in any of the three members' research logs supports the general claim about how long regression suites are re-run. The phase: develop axis assignment is unchanged; it does not depend on this sentence, which only illustrates a principle already established two sentences earlier in the same bullet.
