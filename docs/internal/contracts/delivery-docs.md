...
- **`announcement-internal-comms` (ADR 0059) needed no new verb.** The membership test already admitted it ("announces a unit of product work"). What needed a record is the chain sentence, which enumerates positions and which the 0.1.2 note established a member may not be missing from: the release note announces a unit of work to the people who use it, and the internal announcement tells the rest of the organization. That split is this library's own boundary, drawn from the research rather than quoted from it.
