...
**2026-08-07:** relabelled the section 1 Relationships-section rationale from an unsourced CLAIM ("the most common real-world confusion... duplicate each other") to a POSITION owning the library's own reasoning, per procedure 11. risk-register_research-log.md source [18] (DirectorPM, lines 176-183, mirrored as [8] in raid-log_research-log.md line 115) supports a single bloated artifact attempting both functions, not two standalone documents duplicating each other, and risk-register_research-log.md line 32 treats standalone-register-versus-RAID as a legitimate choice by audience and cadence, not an error. No log entry across the three members' research logs supports a common real-world confusion between a risk log and a performance dashboard; kpi-dashboard_research-log.md line 41 states that relationship as a clean distinction, not a documented failure mode. Obligation unchanged: members still state their position against the other two in the companion's Relationships section.
