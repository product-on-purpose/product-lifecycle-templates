...
| [0036](0036-library-prefix-and-skill-under-skills.md) | **Adopt the `plt-` component prefix and move the skill from the repository root to `skills/plt-fill-template/`**, reaching **convergent** tier on the Advanced Skill Library Standard. Not a compliance chore: the root `SKILL.md` was almost certainly never discoverable, because the Agent Skills spec requires a skill's `name` to match its parent directory and the Claude Code plugin loader scans `skills/`. The plugin name is unchanged; only the component inside it is prefixed. The installed skill's name does change, and whether `npx skills add` now works is still untested | 2026-08-08 |
