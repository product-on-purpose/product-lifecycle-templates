...
target-release: v0.2.0  # the release this plan aimed at; see the banner for what actually shipped
priority: P1
---
...
>
> **Freshness note added 2026-07-26, itself corrected 2026-07-28 (finding DF-5, prose counts drift):** that list of three divergences is itself dated. This file last cites **ADR 0020**; there are now **58** decision records (re-corrected 2026-08-22, 2026-09-20 and 2026-09-23; the 28 this sentence originally carried was stale within days and the 41 that replaced it lasted a month, which is DF-3 recurring twice inside the note written to manage it), and the thirty-six since (0021 through 0056) have changed the taxonomy, the family model, the gate, and the variant model. For what is actually being built and in what order, read [`buildout-specs.md`](buildout-specs.md), not this file. Recorded as finding **DF-3** in STATE.md.
>
> The live roadmap superseding this plan is [`10_roadmap-expanded.md`](roadmap.md) (milestones M0 through M6).
>
> **What happens to this file at v0.2.0 (noted 2026-07-28).** This plan's `target-release` is v0.2.0, and
> that release is imminent. Tagging it does not make the plan current; it closes the milestone the plan was
> written for. At that point `status` moves from `superseded-in-part` to `superseded`, and this file becomes
...
Truthed-up 2026-07-12 against the actual tree. Every row now carries its evidence, per the plan's own rule below: no phase is ticked without a commit, a file, or a passing run to point at.
...
| 2026-07-12 | M0 credibility floor | truth-up | Completion Status table corrected against the tree, with evidence per row (audit finding G-01: the table said "Not started" for all seven phases while P4, the reference bundle, and most of P6, the family, were complete). Recorded the content-first re-sequencing that actually happened: bundles were built before the scaffold (P2) and before the gate port (P3), inverting this plan's order. That inversion was a real choice and it worked, but it was never written down, so the plan and the repo drifted apart within a week. P3 marked superseded (Python gate, not Node); P1 marked done via the eight ADRs. Authority for progress moved to `STATE.md`; this plan is now a dated projection. |

Types: `added`, `re-decomposed`, `phase-completed`, `truth-up`, `superseded`, `closed`.
