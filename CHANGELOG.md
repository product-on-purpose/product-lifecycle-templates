...
- **Specs for two Tier-2 bundles in [`tier2-specs.md`](docs/internal/tier2-specs.md), each written with its
  admission sources retrieved and every quotation checked against the source's raw text** (55 quotations
  and identity checks, all present; four asserted absences confirmed absent). **`issue-log`** has the widest
  named-source base of any Tier-2 spec: PMI's Lexicon, APM's glossary, PRINCE2's glossary and the European
  Commission's PM² guide define it by name, and PM² publishes its field definitions under CC BY 4.0.
  **`definition-of-ready`** is admitted at the practitioner and pattern tier only: neither the 2020 Scrum
  Guide nor SAFe's glossary names it, and the spec says so.
- **[ADR 0057](docs/internal/decisions/0057-issue-log-joins-governance-docs-as-a-fourth-member.md): `issue-log`
  ...
  RAID log's I as the risk register stands to its R.
- **[ADR 0058](docs/internal/decisions/0058-definition-of-ready-joins-standing-standards-as-a-foundation.md):
  `definition-of-ready` joins `standing-standards` as a `foundation`**, the contract moving to `0.3.0`. Its
  failure mode inverts the one the contract names for `foundation`, so its review trigger fires both ways.
