...
**`governance-docs`** (three bundles, the family complete; the first family on the `classification` axis). Standing governance instruments a PM maintains across the whole lifecycle, gated on `classification: utility` rather than a lifecycle phase. Their examples chain on one program (Reporting Platform Modernization) and interlock: the register tracks threats, the RAID log tracks open items, the dashboard tracks whether the delivered program works.
...
| [`kpi-dashboard`](templates/kpi-dashboard/) | The definition of the KPIs that show whether objectives are being met | (none: no governance skill exists yet) |
...
All twelve bundles currently pass. GitHub Actions runs both scripts on every push to `main` and on every pull request ([`.github/workflows/ci.yml`](.github/workflows/ci.yml)), and `main` is branch-protected on the gate, so a bundle that breaks these checks cannot merge.
