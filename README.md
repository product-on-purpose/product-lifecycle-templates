...
```bash
# Clone it. Everything: 31 bundles, the research logs, the gate. Best for reading.
...
# two skills and NOT the 31 bundles, so a skill fetches what it needs on demand
...
<!-- bundle-count: 31 -->
<!-- counts: bundles=31, tier1=25, adrs=58 -->
...
<!-- counts: bundles=31 -->
All thirty-one bundles currently pass. GitHub Actions runs these on every push to `main` and every pull request ([`.github/workflows/ci.yml`](.github/workflows/ci.yml)), and `main` is branch-protected on the gate, so a bundle that breaks these checks cannot merge.
...
<!-- counts: bundles=31, tier1=25, adrs=58, cisteps=32 -->
