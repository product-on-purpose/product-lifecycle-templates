...
<!-- bundle-count: 35 -->
<!-- counts: bundles=35, tier1=25, adrs=60 -->
...
Their examples chain on one fictional "Saved Views" feature, so the family reads as one traceable set: a PRD leads to user stories, ordered in a product backlog and pulled into a sprint backlog, which lead to acceptance criteria, which ship in a release note. A change request is how something already agreed in that chain gets altered, and an internal announcement tells the people who did not do the work what a launch means for them.
...
| [`change-request`](templates/change-request/) | A formal request to alter an agreed baseline, and the decision on it | (none exists yet) |
| [`release-notes`](templates/release-notes/) | The customer-facing announcement of a release | `deliver-release-notes` |
| [`announcement-internal-comms`](templates/announcement-internal-comms/) | The internal announcement of a launch or change: what it means for the reader, and what they must do | (none exists yet) |
...
| **Agent surface** | 2 skills, 3 generated artifacts (`manifest.json`, `sections.json`, `bundle-builds/INDEX.md`), and an MCP server with 5 tools ([ADR 0045](docs/internal/decisions/0045-the-mcp-server-is-python-and-lives-in-this-repository.md)); 71 template variants addressable |
| **Decision records** | 60 ADRs in [MADR v4](https://github.com/adr/madr) ([`docs/internal/decisions/`](docs/internal/decisions/)), all accepted |
...
tools/
  ...
  gen-bundle-build-report.py  Measured build cost per bundle, from harness transcripts
...
bundle-builds/               Measured build-cost reports, from harness transcripts; holds INDEX.md and reports/
docs/
  internal/
