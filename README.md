...
- **Still on credit: "reference implementation".** Thirteen of 205 catalog types (12 of the 27 Tier-1 floor), and **zero fills by anyone but the author**. By the catalog's own tier rule (a type graduates when it "survives one real usage cycle"), nothing here has graduated.
...
<!-- bundle-count: 13 -->
**Thirteen bundles, in four families.** Status `beta`: every one is gate-green and researched, and none has been filled in anger by anyone but the author.
...
### `qa-docs` (one of three bundles; the family is being built)
...
All thirteen bundles currently pass. GitHub Actions runs these on every push to `main` and every pull request ([`.github/workflows/ci.yml`](.github/workflows/ci.yml)), and `main` is branch-protected on the gate, so a bundle that breaks these checks cannot merge.
...
| **Bundles** | 13: 3 complete families (delivery-docs, decision-docs, governance-docs) plus qa-docs in progress |
| **Tier-1 floor** | 12 of the 27 "must-have" types built ([ADR 0021](docs/internal/decisions/0021-complete-the-tier-1-floor.md)) |
