...
<!-- counts: logsgated=29 -->
<!-- counts: sourcesgated=1247 -->
`tools/check-research-logs.py` gates the structural side of this contract in CI (every source entry
has a number, an identity, a tier, and a retrieval status token) across 29 of the library's research
logs and 1,247 individual source entries; the ones it does not cover are named exceptions in the check's
