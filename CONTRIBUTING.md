...
<!-- counts: logsgated=26 -->
<!-- counts: sourcesgated=1126 -->
`tools/check-research-logs.py` gates the structural side of this contract in CI (every source entry
has a number, an identity, a tier, and a retrieval status token) across 26 of the library's research
logs and 1,126 individual source entries; the ones it does not cover are named exceptions in the check's
