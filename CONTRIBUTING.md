...
([`.github/workflows/ci.yml`](.github/workflows/ci.yml)), 32 steps in total, of which four are checkout,
...
<!-- counts: logsgated=27 -->
<!-- counts: sourcesgated=1159 -->
`tools/check-research-logs.py` gates the structural side of this contract in CI (every source entry
has a number, an identity, a tier, and a retrieval status token) across 27 of the library's research
logs and 1,159 individual source entries; the ones it does not cover are named exceptions in the check's
