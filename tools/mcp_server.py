...
template IS the payload, and the largest is roughly 7,200 tokens. So every discovery response carries
...
def axis_values():
    ...
    The axis is `phase` XOR `classification`: 20 bundles carry a phase, 11 a classification, none both
...
def get_grading_pack(bundle_id) -> EnvGrading:
    ...
    Measured: all 30 guides carry a rubric heading (re-checked at 31 on 2026-09-22); FOUR (`okrs`, `product-roadmap`,
