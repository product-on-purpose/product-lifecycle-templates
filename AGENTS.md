...
classification** value, because 20 bundles carry a `phase` and the other 12 carry a `classification`.
Filtering on phase alone silently reaches only 20 of the 32.
...
<!-- counts: bundles=32, sections=278, frontmatter=212 -->
...
fresh in CI) and covers all 32 bundles: 278 sections and 212 frontmatter fill sites. Per bundle, per
...
`npx skills add` route installs the **two skills only**, about 47 KB, and none of the 32 bundles; each
